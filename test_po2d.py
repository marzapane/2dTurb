...
def test_forcing():
    ...
    s_F = po.energy_spectrum(F, np.zeros((1,1)))
    k_F = np.argmax(s_F)
    trgt = N * 25/128
    if (k_F > trgt + 1.5) or (k_F < trgt - 1.5):
        plt.loglog(s_F)
        plt.xlabel('|k|')
...
def test_streamfunction():
    topo = po.random_topography(30, Dx, N)
    rand_om = (10 * np.random.rand(4)).astype(int)
    ...
    if err > 1e-3:
        print(f'--> relative error on q: {err}')
        plt.contourf(topo, levels=50); plt.title('topography'); plt.colorbar(); plt.show()
        plt.contourf(abs(fluid.q-q_bar)); plt.title('err_q'); plt.colorbar(); plt.show()
        plt.contourf(fluid.q); plt.title('q_exp'); plt.colorbar(); plt.show()
        plt.contourf(q_bar); plt.title('q_bar'); plt.colorbar(); plt.show()
    ...
    if err > 1e-14:
        ...
        raise Exception('Error: problem in reconstructing psi.')
